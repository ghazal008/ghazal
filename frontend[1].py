...
roomNo_text = StringVar()
e4 = Entry(window, textvariable=roomNo_text)
...
def search_command():
    clear_list()
    passengers = backend.search(name_text.get(), city_text.get(), year_text.get(), roomNo_text.get())
    insert_list(passengers)
...
def add_command():
    backend.insert(name_text.get(), city_text.get(), year_text.get(), roomNo_text.get())
    view_command()
...
def update_command():
    backend.update(selected_passenger[0], name_text.get(), city_text.get(), year_text.get(), roomNo_text.get())
    view_command()
